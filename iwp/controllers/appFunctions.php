...
function executeRequest($historyID, $type='', $action='', $URL='', $requestData='', $timeout='', $isPluginResponse=true, $callOpt=array()){	
	...
	if(!$cURLErrors->isOk()){
		...
		$multicallArrayCheck = array('6','7', '28', '52', '500', '502', '504','503', 'timeoutClear', 'main_plugin_connection_error');
		if(!isPluginResponse($rawResponseData)){//sometimes 500 error with proper IWP Client Response, so if it not proper response continue set error and exit
			...
			if (!in_array($updateHistoryData['error'], $multicallArrayCheck)) {
...
function updatesNotificationMailSend($force=false){
	...
	foreach($getAllNotifications as $userID => $userData){
		...
		$params = array('userID' => $userID, 'sitesUpdates' => $sitesUpdates,  'updatesNotificationMail' => $updatesNotificationMail, 'updateNotificationDynamicContent' => getOption('updateNotificationDynamicContent'), 'onlyVulnsUpdate' => $onlyVulnsUpdate);
...
function checkTriggerStatus(){
	...
	foreach($data as $key => $value){
		$where = array(
			      		'query' =>  "parentHistoryID = ':parentHistoryID' AND ((status = ':status' AND error IN('7', '6', '28', '52', '500', '502', '504', '503', 'timeoutClear')) OR (status = 'error' AND error IN('main_plugin_connection_error'))) ORDER BY historyID DESC LIMIT 15",
			      		'params' => array(
...
function multiUserGetSitesStatsCount($view, $args, $keysSearch=''){
	if ( $view != 'sites' && $view != 'hiddenUpdates' && userStatus() == "admin") {
		...
		if(userStatus() != "admin"){ 
			...
			$total = DB::getFields('?:hide_list HL, ?:sites S', 'HL.siteID', "S.siteID = HL.siteID ".$wheregroups.$keysSearch." GROUP BY HL.siteID ".$limitSQL);
		}
