...
class manageClientsInstallCloneCommon{
	...
	public static function installCloneCommonNewSitePreProcessor($historyID){
		...
		if(isset($use_sftp) && $use_sftp==1) {
			...
			$uploadPath = '/'.trim($params['remoteFolder'], '/').'/';
			
			$return = self::createCloneDirectory($uploadPath, $connection, $historyID, $params);
			if(!$return){
				return false;
			}
		...
		$destinationURL = removeTrailingSlash($params['newSiteURL'])."/bridge.php";
	...
	public static function installCloneBackupNowProcessor($siteIDs, $params){
		...
		if(!empty($params['isStaging'])){
			$type = "staging";
			$exclude = array(0=>'.user.ini', 1=>'php.ini', 2=>'wordfence-waf.php');
		}
