...
$(function () {
	...
	$(".addSiteButton").live('click',function() {
		...
		if($(".cTypeRadio.active").hasClass('customTxt')){
			...
			if($(".cTypeRadio.active").text()=='application/x-www-form-urlencoded'){
				contentType='';
			}else{
		...
		if(editSite==1){
			...
			if(typeof isStaging!='undefined' && isStaging == 1){
				var saveFtpReqVar = stagingGetSaveFtpReqVar($(this).attr('sid'));
	...
	$("#backupNow").live('click',function() {
		...
		if(backupExcludeFiles=='eg. wp-admin, old-backup.zip'){
	...
	$(".restoreBackup").live('click',function() {
		if($(this).hasClass("needConfirm"))
		{
			if ($(this).hasClass('is_json_notify')) {
				loadConfirmationJsonNotifyPopup($(this));
			}else{
				loadConfirmationPopup($(this));
			}
			return false;
	...
	$(".confirmAction").live('click',function() {
		...
		if($(tempConfirmObject).hasClass('restoreBackup'))
		{
			...
			tempArray['args']['params']={};																	//commented for repeated restore
			...
			$(tempConfirmObject).addClass('disabled');
			$(tempConfirmObject).text('Queued..');
			doHistoryCall(ajaxCallPath,tempArray,"");
			return false;
		} 
		tempConfirmObject = '';
...
$(".toolbar_sites_cont.overflowTabs").live('click',function(){
	...
	$("#saveSettingsBtn").live('click',function(){
		...
		else if(page=="stagingTab")
		{
			...
			var saveFtpReqVar = stagingGetSaveFtpReqVar();
		...
		else if(page=="brandingTab")
		{
			...
			if($(".hideClientPlugin").hasClass('active'))
			{
				tempArray['args']['params']['hide']=1;
			}
			else if($(".doChangesCPB").hasClass('active'))
			{
                            var pluginName = $("#pluginName").val();
                            var authourName = $("#authourName").val();
				...
				var paramsArray={};
				$("#brandingTab .formVal").each(function () {
			...
			if($(".hideUpdatesCPB").hasClass('active')){
				tempArray['args']['params']['hideUpdatesCPB'] = 1;
			}
			if($(".hideFWPCPB").hasClass('active')){
				tempArray['args']['params']['hideFWPCPB'] = 1;
			}
			if($(".doChangesCPB").hasClass('active')){
...
//		doCall(ajaxCallPath,tempArray,"processAppUpdateSettings");
	...
	$("#updateClientConfirm").live('click',function() { 
		...
		tempDataArray['args']['params']['clientUpdatePackage_b64encoded']=clientUpdatesAvailable.clientUpdatePackage;
		tempDataArray['requiredData']={};
	...
	$(".editSiteBtn, .editSiteBtnForFtpSettings").live('click',function() {
		...
		$(".dialog_cont #websiteURL").val(site[$(this).attr('sid')].URL);
		$(".dialog_cont #adminURL").val(site[$(this).attr('sid')].adminURL);
		$(".dialog_cont #username").val(site[$(this).attr('sid')].adminUsername).focus();
		$(".dialog_cont #addSiteSiteName").val(site[$(this).attr('sid')].name).focus();
		$(".dialog_cont .connectURLClass").removeClass('active');
		$(".dialog_cont [def='"+site[$(this).attr('sid')].connectURL+"']").addClass('active');
		...
		if(getPropertyCount(site[$(this).attr('sid')].managerIDs)>0)
		{
			$.each(site[$(this).attr('sid')].managerIDs, function(i, object) {
				$(".addSiteManagers .g"+object).addClass('active');
			});
		}
		
		if(site[$(this).attr('sid')].callOpt!= undefined && (site[$(this).attr('sid')].callOpt.contentType!=''))
		{
			var radionSiteID=site[$(this).attr('sid')];
			...
			{
				$(".cTypeRadio .customTxtVal").val(site[$(this).attr('sid')].callOpt.contentType);
	...
	$('.edit_note').live('click',function(){
		...
		$('<textarea class="edit_site_notes">'+site[siteID]['notes']+'</textarea><i class="save_note">Save Note</i>').insertAfter(site_notes);
		site_notes.hide();
	...
	$('.edit_links').live('click',function(){
		...
		var siteID = $(this).closest('.site_flap_cont_data').attr('btsiteid');
		$('<textarea class="edit_site_links" placeholder="separated by commas">'+site[siteID]['links']+'</textarea><i class="save_links">Save Links</i>').insertAfter(site_links);
		site_links.hide();
		var btmSiteSnap = $('.site_flap_cont_data[btsiteid="'+siteID+'"]');
